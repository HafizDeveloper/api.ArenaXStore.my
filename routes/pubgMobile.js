...
router.post('/', async (req, res) => {
  try {
    ...
    const transactionId = `PUBG${Date.now()}${Math.floor(Math.random() * 9000) + 1000}`;
    ...
    console.error('PUBG Mobile Top-up Error:', error);
    return res.status(500).json({
      success: false,
